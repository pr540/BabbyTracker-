...
async def get_dashboard_data(db: Session, user: User):
    ...
    return {
        ...
        "tasks": [{"id": t.id, "title": t.title, "is_completed": t.is_completed, "due_time": t.due_time, "category": t.category} for t in tasks],
...
async def register_baby(
    ...
    user = db.query(User).filter(User.id == user_id).first()
    if not user or user.baby:
        raise HTTPException(status_code=400, detail="User already has a baby or not found")
    ...
    return JSONResponse({"status": "success", "baby_id": new_baby.id})
